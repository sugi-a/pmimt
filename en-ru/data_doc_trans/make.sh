...
for f in $tmpd/min_filtered/train-*.src; do
    cat $f \
...
for f in $tmpd/min_filtered/train-*.trg; do
    cat $f \
        | awk -v SOS="$SOS" -v EOS="$EOS" -v N=$N -f ./wrap_trg.awk \
        > $tmpd/concat/$(basename $f) &
done
wait
...
for srcf in $tmpd/concat/train-*.src; do
    ...
    paste $srcf $trgf \
        ...
        | tee >(cut -f 1 | sed -r -e 's| <SEP> |\t|' > ./$bname.src) \
        | cut -f 2 > ./$bname.trg &
done
wait
